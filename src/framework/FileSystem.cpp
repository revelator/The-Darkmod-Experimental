...
const char *idFileSystemLocal::OSPathToRelativePath( const char *OSPath ) {
	...
	for( int gpath = 0; gpath < GPATH_COUNT; gpath++ ) {
		switch( gpath ) {
		case 0:
			gamePath = BASE_GAMEDIR;
			break; // taaaki - seems to be some issues with removing this - need to look into it further
		case 1:
			gamePath = dynbase.c_str();
			break; // base the tdm directory off the bin path instead of hardcoding it
		case 2:
			gamePath = fs_mod.GetString();
			break;
		case 3:
			gamePath = fs_currentfm.GetString();
			break; // taaaki - may need to check this if my assumptions are incorrect
		default:
			gamePath = NULL;
			break;
		}
