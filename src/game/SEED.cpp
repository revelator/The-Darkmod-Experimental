...
void Seed::AddClassFromEntity( idEntity *ent, const bool watch, const bool getSpawnargs ) {
	...
	if( SeedClass.sink_max < SeedClass.sink_min ) {
		SeedClass.sink_max = SeedClass.sink_min;
	}
	...
	if( SeedClass.scale_max.x < SeedClass.scale_min.x ) {
		SeedClass.scale_max.x = SeedClass.scale_min.x;
	}
	if( SeedClass.scale_max.y < SeedClass.scale_min.y ) {
		SeedClass.scale_max.y = SeedClass.scale_min.y;
	}
	if( SeedClass.scale_max.z < SeedClass.scale_min.z ) {
		SeedClass.scale_max.z = SeedClass.scale_min.z;
	}
	SeedClass.func_x = 0;
	SeedClass.func_y = 0;
	SeedClass.func_s = 0;
...
void Seed::PrepareEntities( void ) {
	...
	for( int i = 0; i < s; i++ ) {
		...
		int temp = ClassIndex[i];
		ClassIndex[i] = ClassIndex[second];
		ClassIndex[second] = temp;
	}
		...
		for( int j = 0; j < iEntities; j++ ) {
			...
			while( tries++ < MAX_TRIES ) {
				...
				if( m_Classes[i].materials.Num() > 0 ) {
					...
					idVec3 traceEnd = SeedEntity.origin;
					traceEnd.z = m_origin.z - size.z / 2;
