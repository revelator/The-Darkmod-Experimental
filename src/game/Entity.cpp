	...
	for( int i = 1; i < LOD_LEVELS; i++ ) {
		...
		if( m_LOD->DistLODSq[i] > 0 ) {
			...
			if( i < LOD_LEVELS - 1 ) {
				...
				if( m_LOD->ModelLOD[i].Length() == 0 ) {
					m_LOD->ModelLOD[i] = m_LOD->ModelLOD[0];
				}
				sprintf( temp, "skin_lod_%i", i );
				m_LOD->SkinLOD[i] = dict->GetString( temp );
				if( m_LOD->SkinLOD[i].Length() == 0 ) {
					m_LOD->SkinLOD[i] = m_LOD->SkinLOD[0];
				}
...
idEntity::~idEntity( void ) {
	...
	for( int i = 0; i < m_objLocations.Num(); ++i ) {
		...
		if( locationEnt == NULL ) {
			continue;    // probably already deleted
		}
...
void idEntity::SetFrobHighlightState( const bool newState ) {
	if( m_bFrobHighlightState == newState ) {
...
void idEntity::FrobAction( bool frobMaster, bool isFrobPeerAction ) {
	if( m_FrobActionLock ) {
		return;    // prevent double-entering this function
	}
	if( IsHidden() ) {
		return;    // don't do frobactions on hidden entities
	}
...
void idEntity::SetFrobbed( const bool val ) {
	if( m_bFrobbed == val ) {
		return;    // avoid loopbacks and unnecessary work
	}
	m_bFrobbed = val;
	// resolve the peer names into entities
	for( int i = 0; i < m_FrobPeers.Num(); i++ ) {
		if( m_FrobPeers[i].Length() == 0 ) {
			continue;
		}
		idEntity *ent = gameLocal.FindEntity( m_FrobPeers[i] );
