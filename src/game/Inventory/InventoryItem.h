...
public:
	...
	CInventoryCategory		*Category() const {
		return m_Category;
	}
	void					SetCategory( CInventoryCategory *newCategory ) {
	...
	void					SetItemEntity( idEntity *ent ) {
		m_Item = ent;
	};
	idEntity				*GetItemEntity() {
		return m_Item.GetEntity();
	}
	...
	void					SetType( CInventoryItem::ItemType type ) {
		m_Type = type;
	};
	ItemType				GetType() {
		return m_Type;
	...
	void					SetItemId( const idStr &id ) {
		m_ItemId = id;
	};
	const idStr			&GetItemId() {
		return m_ItemId;
	};
