...
int tdmEAS::GetAreaNumForPosition( const idVec3 &position ) {
	...
	if( areaNum == 0 ) {
		areaNum = _aas->file->PointReachableAreaNum( position + idVec3( 0, 0, 16 ), _aas->DefaultSearchBounds(), AREA_REACHABLE_WALK, 0 );
	}
	if( areaNum == 0 ) {
		areaNum = _aas->file->PointReachableAreaNum( position + idVec3( 0, 0, 32 ), _aas->DefaultSearchBounds(), AREA_REACHABLE_WALK, 0 );
	}
	if( areaNum == 0 ) {
		areaNum = _aas->file->PointReachableAreaNum( position + idVec3( 0, 0, 48 ), _aas->DefaultSearchBounds(), AREA_REACHABLE_WALK, 0 );
	}
	return areaNum;
}
