...
void idAI::InitMuzzleFlash( void ) {
	...
	worldMuzzleFlash.shaderParms[ SHADERPARM_TIMESCALE ] = 1.0f; // V519 PVS Studio (shaderparms 3 is used twice is this a mistake ?)
...
idActor *idAI::FindNearestEnemy( bool useFOV ) {
	...
	dist = delta.LengthSqr();
	// V640 PVS Studio (need brackets here to avoid confusion)
	if( dist < bestDist ) {
...
void idAI::ShowDebugInfo() {
	...
	if( cv_ai_task_show.GetBool() ) {
		...
		if( GetSubsystem( ai::SubsysSenses )->IsEnabled() ) {
			str += "Senses: " + GetSubsystem( ai::SubsysSenses )->GetDebugInfo() + "\n";
		}
		if( GetSubsystem( ai::SubsysMovement )->IsEnabled() ) {
			str += "Movement: " + GetSubsystem( ai::SubsysMovement )->GetDebugInfo() + "\n";
		}
		if( GetSubsystem( ai::SubsysCommunication )->IsEnabled() ) {
			str += "Comm: " + GetSubsystem( ai::SubsysCommunication )->GetDebugInfo() + "\n";
		}
		if( GetSubsystem( ai::SubsysAction )->IsEnabled() ) {
			str += "Action: " + GetSubsystem( ai::SubsysAction )->GetDebugInfo() + "\n";
		}
...
void idAI::SwapHeadAFCM( bool bUseLargerCM ) {
	if( m_HeadJointID == -1 ) {
		return;    // safety check for AI without head
	}
