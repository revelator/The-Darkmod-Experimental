...
const idMD5Anim *idAnim::MD5Anim( int num ) const {
	// changed to the pointer of anims revelator
...
const char *idAnim::AddFrameCommand( const idDeclModelDef *modelDef, int framenum, idLexer &src, const idDict *def, animFlags_t *flags ) { // grayman #3182
	...
	} else if( token == "muzzle_flash" ) { // V517 PVS Studio (yet another one)
		if( !src.ReadTokenOnLine( &token ) ) {
			return "Unexpected end of line";
		}
	...
	while( frame != to ) {
		...
		while( index < end ) {
			...
			case FC_PICKUP: {
				...
				idStr Spawnarg = "pickup_";
				Spawnarg.Append( name );
...
bool idAnimator::CreateFrame( int currentTime, bool force ) {
	...
	if( cv_ai_opt_noanims.GetBool() && entity->CheckDormant() ) {
		return false;
	}
